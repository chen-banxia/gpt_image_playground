...
function drawMaskImageToCanvas(maskImage: HTMLImageElement, maskCanvas: HTMLCanvasElement) {
  ...
  if (Math.abs(maskAspect - canvasAspect) > 0.001) {
    throw new Error(tCurrent('maskSizeMismatchCurrent'))
  }
...
export default function MaskEditorModal() {
  ...
  const handleRemoveMask = () => {
    setConfirmDialog({
      title: t('removeMask'),
      message: t('removeMaskMessage'),
      tone: 'danger',
      action: () => {
        clearMaskDraft()
        setMaskEditorImageId(null)
        showToast(t('maskRemoved'), 'success')
      },
    })
  }
  ...
  useEffect(() => {
    ...
    async function loadCanvases() {
      try {
        ...
        if (cancelled) return
        if (!dataUrl) {
          showToast(t('imageMissingCannotEditMask'), 'error')
          setMaskEditorImageId(null)
          return
        }
        ...
        if (!imageCtx) throw new Error(t('canvasUnsupported'))
        ...
        if (maskDraft?.targetImageId === targetImageId) {
          try {
            ...
            fillWhiteMask(maskCanvas)
            showToast(
              t('maskDraftLoadFailed', { message: err instanceof Error ? err.message : String(err) }),
              'error',
            )
          }
        }
        ...
        if (preparedTarget.wasResized) {
          showToast(
            t('maskResizeNotice', {
              from: `${preparedTarget.originalWidth}x${preparedTarget.originalHeight}`,
              to: `${preparedTarget.width}x${preparedTarget.height}`,
            }),
            'info',
          )
        }
    ...
    <>
      ...
      <div className="flex-none flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 z-20">
        <div className="flex items-center gap-3">
          <button onClick={close} disabled={isSaving} className="p-2 -ml-2 text-gray-500 hover:bg-gray-100 rounded-lg dark:text-gray-400 dark:hover:bg-gray-800 transition" title={t('cancel')}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
          <div className="relative flex items-center gap-1.5">
            <h2 className="text-sm font-medium text-gray-700 dark:text-gray-200" id="mask-editor-title">{t('maskEditorTitle')}</h2>
            <button
              type="button"
        ...
        <div className="absolute bottom-4 sm:bottom-8 left-1/2 -translate-x-1/2 flex items-center justify-center z-20 pointer-events-none w-full px-2 sm:px-4">
          <div className="flex items-center gap-2 sm:gap-4 px-2 sm:px-3 py-1.5 sm:py-2 bg-white/95 dark:bg-[#0f0f0f]/95 backdrop-blur-md border border-gray-200/80 dark:border-white/5 rounded-2xl sm:rounded-[1.25rem] shadow-2xl pointer-events-auto">
            ...
            <div className="flex items-center gap-0.5 sm:gap-2 sm:ml-1">
              <button onClick={handleUndo} disabled={!canUndo} className="p-2 sm:p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl disabled:opacity-30 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200 transition-all" title={t('undo')}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M3 7v6h6" />
                  <path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3L3 13" />
                </svg>
              </button>
              <button onClick={handleRedo} disabled={!canRedo} className="p-2 sm:p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl disabled:opacity-30 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200 transition-all" title={t('redo')}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21 7v6h-6" />
              ...
              <button onClick={resetViewTransform} disabled={!isReady || isSaving || !isZoomed} className="p-2 sm:p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl disabled:opacity-30 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200 transition-all" title={t('resetView')}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M4 14h6v6"/>
                  <path d="M20 10h-6V4"/>
                  <path d="M14 10l7-7"/>
              ...
              <button onClick={handleClear} disabled={!isReady || isSaving} className="p-2 sm:p-2.5 text-gray-500 hover:bg-gray-100 rounded-lg sm:rounded-xl disabled:opacity-30 dark:text-[#8a8a8e] dark:hover:bg-white/10 dark:hover:text-gray-200 transition-all" title={t('clearMask')}>
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M3 6h18"/>
