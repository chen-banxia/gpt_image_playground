    ...
    <div data-no-drag-select className="fixed inset-0 z-[70] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100">{t('setImageSize')}</h3>
            <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">{t('currentValue', { value: currentSize || 'auto' })}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-6">
          ...
          <div className="min-h-[220px]">
            {mode === 'auto' && (
              <div className="flex h-full animate-fade-in items-center justify-center pt-8 pb-4 text-center">
                <div>
                  ...
                  <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200">{t('autoSize')}</h4>
                  <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">{t('autoSizeHint')}</p>
                </div>
              </div>
            )}
            ...
            {mode === 'ratio' && (
              <div className="space-y-5 animate-fade-in">
                <section>
                  <div className="mb-2 text-xs font-medium text-gray-400 dark:text-gray-500">{t('baseResolution')}</div>
                ...
                <section>
                  <div className="mb-2 text-xs font-medium text-gray-400 dark:text-gray-500">{t('imageRatio')}</div>
                ...
                {ratio === 'custom' && (
                  <label className="block animate-fade-in">
                    <span className="mb-2 block text-xs font-medium text-gray-400 dark:text-gray-500">{t('enterCustomRatio')}</span>
                    <input
                      value={customRatio}
            ...
            {mode === 'resolution' && (
              <div className="space-y-5 animate-fade-in">
                <section>
                  <div className="mb-4 text-xs font-medium text-gray-400 dark:text-gray-500">{t('enterPixels')}</div>
                  <div className="flex items-center gap-4">
                    <label className="flex-1">
                      <span className="mb-1.5 block text-xs text-gray-500 dark:text-gray-400">{t('width')} (Width)</span>
                      <input
                        type="number"
                        value={customW}
                    ...
                    <label className="flex-1">
                      <span className="mb-1.5 block text-xs text-gray-500 dark:text-gray-400">{t('height')} (Height)</span>
                      <input
                        type="number"
                        value={customH}
          ...
          <div className="rounded-2xl bg-gray-50 px-4 py-3 dark:bg-white/[0.03]">
            <div className="text-xs text-gray-400 dark:text-gray-500">{t('willUse')}</div>
            <div className="mt-1 flex items-center gap-2">
              <span className="font-mono text-lg font-semibold text-gray-800 dark:text-gray-100">
                {previewSize || t('invalidSize')}
              </span>
              {isClamped && (
                <div
